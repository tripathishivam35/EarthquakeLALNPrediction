...
print(os.listdir("/Users/shivam/Documents/Data science/data/kaggle/LANL-Earthquake-Prediction/"))
...
df = pd.read_csv('/Users/shivam/Documents/Data science/data/kaggle/LANL-Earthquake-Prediction/train.csv', dtype={'acoustic_data': np.int16,'time_to_failure': np.float32}).values
...
def sub_table(index, seg_len):
    ...
    X = pd.read_csv('/Users/shivam/Documents/Data science/data/kaggle/LANL-Earthquake-Prediction/train.csv', dtype={'acoustic_data': np.int16},
        usecols=['acoustic_data'], skiprows =range(1, index-1), nrows=seg_len)
        
    #We pick up the last 'time_to_failure' as a prediction variable
    y = pd.read_csv('/Users/shivam/Documents/Data science/data/kaggle/LANL-Earthquake-Prediction/train.csv', dtype={'time_to_failure': np.float32},
...
#model = CatBoostRegressor(iterations = 2,learning_rate = 1,depth = 2).fit(train.drop('y',axis=1),train['y'])
test = []
#We load all test sets in a list
for file in glob.glob("/Users/shivam/Documents/Data science/data/kaggle/LANL-Earthquake-Prediction/test/*.csv"):
    test.append(file.split('/')[-1].split('.')[0])

Test = []
for csv in test:
    Test.append(mk_features(pd.read_csv(f'/Users/shivam/Documents/Data science/data/kaggle/LANL-Earthquake-Prediction/test/{csv}.csv').values))
...
submit.to_csv('shivam_1May_earthquake.csv', index=False)
